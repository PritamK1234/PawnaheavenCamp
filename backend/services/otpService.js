...
function validateMobile(mobile) {
  ...
  // remove country code only if length is 12
  if (cleaned.length === 12 && cleaned.startsWith("91")) {
    number = cleaned.substring(2);
  }

  // if already 10 digits keep it
  if (number.length !== 10) {
    return null;
  }
...
const OtpService = {
  async sendOtp(mobile, purpose) {
    ...
    if (
      !["owner_login", "referral_login", "referral_register"].includes(purpose)
    ) {
    ...
    try {
      const templateParam = MSG91_TEMPLATE_ID
