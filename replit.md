...
- **Ledger entry on confirm**: `bookingController.js` CONFIRM branch inserts a ledger entry so camping calendar doesn't double-count persons from bookings table.
- **New `booking_id` column** on `ledger_entries` table (VARCHAR 50, indexed) — links ledger entries to booking for deduplication.
...
- **Files changed**: `backend/schema.sql`, `backend/controllers/bookingController.js`, `backend/controllers/propertyController.js`, `backend/controllers/villa/villaController.js`, `backend/controllers/camping/camping_CottagesController.js`, `src/components/CalendarSync.tsx`
...
- **Payment Gateway**: Paytm Custom Checkout JS SDK (STAGING env). Opens payment overlay on our page — no redirect to Paytm, no "Processing" page hang.
- **Payment Flow**: BookingForm closes dialog → 200ms delay → calls `PaytmPaymentService.openCheckout(bookingId)` → backend returns txnToken → Paytm JS SDK opens overlay on main page (no focus trap) → on success, frontend POSTs txnResponse to `/api/payments/verify/:booking_id` → backend verifies via Paytm Status API (with SDK callback fallback for staging) → updates DB → sends WhatsApp via `sendBookingNotifications()`.
- **Paytm SDK Loading**: Dynamic script loading in `src/lib/paytmPayment.ts` from `securestage.paytmpayments.com/merchantpgpui/checkoutjs/merchants/{MID}.js`
- **Paytm SDK Callback Fallback** (Feb 2026): When Paytm Status API returns TXN_FAILURE but SDK callback says TXN_SUCCESS, the verify endpoint trusts the SDK data — with checksum verification in production, or unconditionally in staging mode. This fixes staging test payments.
- **Legacy Redirect Fallback**: `PaytmPaymentService.redirectToPaytm()` and `initiateAndRedirect()` still available as fallback methods
- **Payment Callback (backup)**: POST `/api/payments/paytm/callback` still handles server-to-server Paytm callbacks with checksum verification (backup for when Paytm sends webhook)
- **Payment Verification**: GET/POST `/api/payments/verify/:booking_id` — checks Paytm Status API, falls back to SDK callback data, updates DB, sends all 3 WhatsApp notifications. Idempotent — skips if payment_status already SUCCESS.
...
- **WhatsApp Service** (Feb 2026): `backend/utils/whatsappService.js` — centralized service with `sendTextMessage()`, `sendInteractiveButtons()`, `sendTemplateMessage()`, and `sendBookingNotifications()`. All methods use `callWhatsAppAPI()` with full error logging (HTTP status, error code, error message). Phone numbers auto-formatted (10-digit → +91 prefix, removes +). For production: create templates `booking_confirmation`, `owner_new_booking`, `admin_booking_alert` in Meta Business Manager. Test with built-in `hello_world` template.
