...
const OwnerProfile = () => {
              ...
              if (typeof item === "string") {
                ...
                if (trimmedItem.startsWith("{") && trimmedItem.endsWith("}")) {
                  try {
                    ...
                    return {
                      time: parsed.time || "",
                      title: parsed.title || parsed.event || "",
                    };
                  } catch (e) {
                ...
                try {
                  ...
                  return typeof parsedItem === "object"
                    ? parsedItem
                    : { time: "", title: parsedItem };
                } catch (e) {
                  return { time: "", title: trimmedItem };
                }
              }
  ...
  const addItem = (type: string) => {
    if (type === "schedule") {
      ...
      setDetails({
        ...
        schedule: [
          ...(details.schedule || []),
          { time: newItem.time.trim(), title: newItem.value.trim() },
        ],
      });
    } else {
  ...
  const TagList = ({
    ...
    type: "activities" | "highlights" | "policies";
    items: string[];
  }) => (
    <div className="space-y-3">
      <div className="flex flex-wrap gap-2">
        {items.map((item, idx) => (
          <div
            key={idx}
            className="flex items-center bg-black/40 border border-[#D4AF37]/30 px-3 py-1.5 rounded-md group"
          >
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-6">
        <div className="space-y-3">
          <Label className="text-sm font-bold capitalize tracking-widest text-gray-400">
            Activities
          </Label>
        ...
        {/* Schedule Section */}
        <div className="space-y-3">
          <Label className="text-sm font-bold capitalize tracking-widest text-gray-400 flex items-center gap-2">
          ...
          <div className="space-y-3">
            {(details.schedule || []).map((item, idx) => (
              <div
                key={idx}
                className="flex items-center justify-between bg-black/40 border border-[#D4AF37]/20 p-3 rounded-xl group"
              >
                <div className="flex items-center gap-3">
                  <div className="w-2 h-2 rounded-full bg-[#D4AF37]" />
                  <div>
                    <p className="text-[10px] font-bold text-[#D4AF37] capitalize">
                      {item.time}
                    </p>
            ...
            <div className="p-4 border border-dashed border-[#D4AF37]/30 rounded-xl space-y-3 bg-[#1A1A1A]/50">
              <p className="text-[10px] font-bold text-gray-500 capitalize tracking-widest">
                Add New Event
              </p>
        ...
        <Button
          onClick={handleSave}
          disabled={loading}
          className="w-full bg-[#D4AF37] hover:bg-[#B8860B] text-black font-bold h-12 shadow-xl"
        >
